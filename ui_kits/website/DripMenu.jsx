...
function DripMenu({ onNavigate }) {
  const { Button, Icon, SectionLabel, DripRow, BoosterRow, Chip } = DSm;
    ...
    <div>
      <section style={{ background: 'var(--navy-900)', padding: 'clamp(56px,6vw,96px) var(--gutter) 0' }}>
        <div style={{ maxWidth: 'var(--container)', margin: '0 auto' }}>
          ...
          <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 300, fontSize: 'var(--fs-d1)', color: 'var(--ivory)', letterSpacing: '-0.02em', margin: '20px 0 0', maxWidth: '20ch' }}>Signature drips, priced two ways: Standard and Premium.</h1>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', margin: '40px 0 8px' }}>
            {cats.map(c => <Chip key={c} selected={filter === c} onClick={() => setFilter(c)}>{c}</Chip>)}
          </div>
      ...
      <section style={{ background: 'var(--navy-800)', padding: 'var(--section-y) var(--gutter)' }}>
        <div style={{ maxWidth: 'var(--container)', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
            {injections.map(([n, b, p]) => <BoosterRow key={n} name={n} bestFor={b} price={p} />)}
          </div>

          <div style={{ height: 1, background: 'var(--gold-a40)', margin: '56px 0' }} />

          <SectionLabel>IV Boosters</SectionLabel>
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 300, fontSize: 'var(--fs-d2)', color: 'var(--ivory)', letterSpacing: '-0.02em', margin: '16px 0 32px' }}>Vitamins, antioxidants &amp; aminos</h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
            {boosters.map(([n, b, p]) => <BoosterRow key={n} name={n} bestFor={b} price={p} />)}
          </div>
        </div>
      </section>
      ...
      <section style={{ background: 'var(--ivory)', padding: 'var(--section-y-tight) var(--gutter)' }}>
        <div style={{ maxWidth: 'var(--container-text)', margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--slate-ink)', fontSize: 'var(--fs-lg)', margin: '16px auto 30px', maxWidth: '46ch' }}>Book a consultation and Rahab will tailor a drip to your goals.</p>
