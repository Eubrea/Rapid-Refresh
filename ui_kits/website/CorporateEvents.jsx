...
const pillCE = { borderRadius: 'var(--radius-pill)' };

function CorporateEvents({ onNavigate, onBook }) {
  const { Button, Icon, SectionLabel } = DSce;
  ...
  return (
    <div>
      ...
      <section style={{ background: 'var(--navy-900)', padding: 'clamp(56px,6vw,104px) var(--gutter)' }}>
        <div style={{ maxWidth: 'var(--container)', margin: '0 auto', display: 'grid', gridTemplateColumns: '1.05fr 0.95fr', gap: 'clamp(32px,5vw,72px)', alignItems: 'center' }}>
          <div>
            ...
            <h1 style={{ fontFamily: 'var(--font-display)', fontWeight: 300, fontSize: 'var(--fs-d1)', color: 'var(--ivory)', letterSpacing: '-0.02em', margin: '20px 0 0', maxWidth: '18ch' }}>Group wellness, delivered in style.</h1>
            ...
            <div style={{ display: 'flex', gap: 14, marginTop: 34, flexWrap: 'wrap' }}>
              <Button variant="primary" size="lg" style={pillCE} onClick={onBook}>Request Group Pricing</Button>
              <Button variant="secondary" size="lg" style={pillCE} onClick={() => onNavigate('menu')}>View the Drip Menu</Button>
            </div>
          </div>
          <Reveal><Photo src="../../assets/photos/corporate-hero.webp" alt="Corporate wellness day" caption="Corporate wellness day" ratio="4 / 5" radius="var(--radius-xl)" /></Reveal>
        </div>
      </section>

      {/* Two tracks */}
      <section style={{ background: 'var(--ivory)', padding: 'var(--section-y) var(--gutter)' }}>
        <div style={{ maxWidth: 'var(--container)', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 26 }}>
          {[['briefcase', 'Corporate Wellness', 'On-site hydration that keeps teams sharp, energized, and cared for.', corporate, 'var(--ivory-100)'],
            ['gem', 'Private Events', 'A glowing, memorable wellness moment for your celebration.', events, 'var(--gold-a16)']].map(([ic, title, blurb, list, tint], i) => (
            <Reveal key={title} delay={i * 90}>
              <div style={{ background: tint, borderRadius: 'var(--radius-xl)', padding: 'clamp(28px,3vw,44px)', height: '100%' }}>
                <span style={{ width: 58, height: 58, borderRadius: '50%', background: 'var(--ivory)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gold-600)', boxShadow: 'var(--shadow-sm)' }}><Icon name={ic} size={28} color="currentColor" /></span>
                ...
                <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {list.map((item, j) => (
                    <li key={item} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 0', borderBottom: j < list.length - 1 ? '1px solid rgba(58,65,79,0.12)' : 'none' }}>
                      <span style={{ width: 26, height: 26, flex: '0 0 auto', borderRadius: '50%', background: 'var(--ivory)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: 'var(--gold-600)', boxShadow: 'var(--shadow-xs)' }}><Icon name="check" size={14} color="currentColor" /></span>
            ...
            {perks.map(([ic, t, d], i) => (
              <Reveal key={t} delay={i * 70}>
                <div style={{ background: 'var(--ivory)', borderRadius: 'var(--radius-xl)', padding: '30px 26px', height: '100%', boxShadow: 'var(--shadow-sm)' }}>
                  <span style={{ color: 'var(--gold-600)' }}><Icon name={ic} size={28} color="currentColor" /></span>
      ...
      <section style={{ background: 'var(--ivory)', padding: 'var(--section-y) var(--gutter)' }}>
        <div style={{ maxWidth: 'var(--container)', margin: '0 auto' }}>
          <div style={{ background: 'var(--navy-800)', borderRadius: 'var(--radius-xl)', padding: 'clamp(44px,5vw,76px) clamp(28px,5vw,68px)', display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 40, alignItems: 'center', boxShadow: 'var(--shadow-lift)' }}>
            <div>
              <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 300, fontSize: 'var(--fs-d2)', color: 'var(--ivory)', letterSpacing: '-0.02em', margin: 0 }}>Planning something? Let’s tailor it.</h2>
              <p style={{ color: 'var(--ivory-a64)', fontSize: 'var(--fs-lg)', margin: '16px 0 0', maxWidth: '48ch' }}>Tell us your group size, date, and location, and we’ll build a custom quote.</p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <Button variant="primary" size="lg" style={pillCE} onClick={onBook}>Request Group Pricing</Button>
              <a href="tel:+17138517590" style={{ textDecoration: 'none' }}><Button variant="ghost" size="lg" fullWidth style={pillCE} iconLeft={<Icon name="phone" size={17} color="currentColor" />}>(713) 851-7590</Button></a>
            </div>
          </div>
        </div>
      </section>
