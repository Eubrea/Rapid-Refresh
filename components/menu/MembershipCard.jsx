...
export function MembershipCard({
  name, price, cadence = '/month', tagline, features = [],
  cta = 'Become a Member', featured = false, onSelect, style = {},
}) {
  return (
    <div style={{
      ...
      <span style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginTop: '16px' }}>
        <span style={{ fontFamily: 'var(--font-display)', fontSize: '3rem', fontWeight: 'var(--fw-light)', letterSpacing: '-0.02em', color: 'var(--ivory)', lineHeight: 1 }}>${price}</span>
        <span style={{ fontFamily: 'var(--font-text)', fontSize: 'var(--fs-sm)', color: 'var(--ivory-a44)' }}>{cadence}</span>
      </span>

      {tagline && <p style={{ fontFamily: 'var(--font-text)', fontSize: 'var(--fs-sm)', color: 'var(--ivory-a64)', marginTop: '12px', lineHeight: 1.55 }}>{tagline}</p>}
      ...
      <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: '14px', flex: 1 }}>
        {features.map((f, i) => (
          <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', fontFamily: 'var(--font-text)', fontSize: 'var(--fs-sm)', color: 'var(--ivory-a80)', lineHeight: 1.45 }}>
            <span style={{ color: 'var(--gold-400)', marginTop: 1, flex: '0 0 auto', display: 'inline-flex' }}><Icon name="check" size={17} /></span>
            {f}
          </li>
        ))}
      </ul>

      <Button variant={featured ? 'primary' : 'secondary'} fullWidth onClick={onSelect} style={{ marginTop: '30px' }}>{cta}</Button>
    </div>
  );
}
